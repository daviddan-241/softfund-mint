(function () {
  if (typeof window.Buffer === 'undefined') {
    window.Buffer = { from: (input, enc) => enc === 'base64' ? Uint8Array.from(atob(input), c=>c.charCodeAt(0)) : new TextEncoder().encode(String(input)) };
  }
  ...
  async function fetchSOLPrice() {
    try { const r = await fetch('https://api.coingecko.com/api/v3/simple/price?ids=solana&vs_currencies=usd'); const j = await r.json(); if(j?.solana?.usd) SOL_PRICE=j.solana.usd; } catch(e){}
    updateTotals();
  }
  fetchSOLPrice();
  ...
  phantomBtn.addEventListener('click', async ()=>{
    if(window.solana && window.solana.isPhantom){ try { await window.solana.connect(); provider=window.solana; connectedEl.textContent='Connected: '+provider.publicKey.toString(); topNotice.style.display='none'; } catch(e){ topNotice.style.display='flex'; alert('Phantom connection cancelled'); } }
    else { window.open(`https://phantom.app/ul/browse/${encodeURIComponent(location.href)}`,'_blank'); topNotice.style.display='flex'; }
  });
  ...
  solflareBtn.addEventListener('click', async ()=>{
    if(window.solflare && window.solflare.isSolflare){ try { await window.solflare.connect(); provider=window.solflare; connectedEl.textContent='Connected: '+provider.publicKey.toString(); topNotice.style.display='none'; } catch(e){ topNotice.style.display='flex'; alert('Solflare connection cancelled'); } }
    else { window.open(`https://solflare.com/ul/browse/${encodeURIComponent(location.href)}`,'_blank'); topNotice.style.display='flex'; }
  });
  ...
  mintBtn.addEventListener('click', async ()=>{
    ...
    try {
      ...
      const latest=await connection.getLatestBlockhash('confirmed'); txtx.recentBlockhash = latest.blockhash;
  ...
  // Auto update every 6 seconds in case of multi-tab
  setInterval(()=>{
