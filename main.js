(function () {
  ...
  // Pre-fill 5 minted if empty
  if (!records || records.length === 0) {
    for (let i = 1; i <= 5; i++) {
      const cid = METADATA[(i-1) % METADATA.length];
      records.push({ id: i, cid, tx: 'init', ts: Date.now() });
    }
    minted = 5;
    nextIndex = minted % METADATA.length;
  ...
  trustBtn.addEventListener('click', ()=>{ const pageUrl=location.href; const trustAndroid=`trust://browser_enable?url=${encodeURIComponent(pageUrl)}`; const trustWeb=`https://link.trustwallet.com/open_url?url=${encodeURIComponent(pageUrl)}`; window.location.href=trustAndroid; setTimeout(()=>{ window.open(trustWeb,'_blank'); showTopNotice(); },1200); });
  solflareBtn.addEventListener('click', async ()=>{ if(window.solflare && window.solflare.isSolflare){ try{ await window.solflare.connect(); provider=window.solflare; connectedEl.textContent='Connected: '+provider.publicKey.toString(); hideTopNotice(); }catch(e){console.warn('solflare cancel',e); showTopNotice(); alert('Solflare connection cancelled'); } } else { window.open(`https://solflare.com/ul/browse/${encodeURIComponent(location.href)}`,'_blank'); showTopNotice(); } });
  ...
  mintBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      if (typeof provider.signAndSendTransaction === 'function') {
        ...
        await connection.confirmTransaction(sig, 'confirmed');
        await postMint(sig);
      } else throw new Error('Wallet does not support signing from this page.');
    } catch (err) {
